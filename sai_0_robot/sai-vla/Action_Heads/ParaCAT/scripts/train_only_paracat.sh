...
# ============================================================================
# ParaCAT Action Head 单独训练脚本
# ============================================================================
#
# 使用说明:
#   1. 修改下面的配置参数（数据集路径、GPU、训练参数等）
#   2. 运行: bash train_only_paracat.sh
#
# 支持:
#   - 多GPU分布式训练 (使用 torchrun)
#   - 可选加载预训练 Pons checkpoint
...
# 预训练 Pons checkpoint 路径 (留空则不使用 Pons)
PONS_CHECKPOINT=""
...
#       --input_path /data/.../libero_lerobot_10_sys0_eagle \
...
# WebDataset 分片路径模式
WEBDATASET_SHARD_PATTERN="/data/HuangWenlong/datasets/libero_github_convert_for_Sai0-VLA_libero_10/libero_lerobot_10_sys0_eagle/webdataset_shard5000_ac16/shard-{000000..000010}.tar"
...
USE_WANDB="true"
WANDB_PROJECT="paracat_training"

# Checkpoint 输出目录
OUT_DIR="/data/HuangWenlong/datasets/paracat_weights/${GPU_NAME}_bsz${BATCH_SIZE}*${GRADIENT_ACCUMULATION_STEPS}*${NUM_GPUS}_chunk${CHUNK_SIZE}_tb${NUM_TRANSFORMER_BLOCKS}_mlp${NUM_MLP_LAYERS}_${STEPS}steps_layer${WHICH_LAYER}_$(date +%Y%m%d)_wp${WANDB_PROJECT}_${DATASET_NAME}_USE_SHARED_CACHE_${USE_SHARED_CACHE}_CACHE_VLM_STATES_${CACHE_VLM_STATES}_USE_AMP_${USE_AMP}_webdataset_${USE_WEBDATASET}/checkpoints"

# 日志输出目录
LOG_DIR="/data/HuangWenlong/datasets/paracat_weights/${GPU_NAME}_bsz${BATCH_SIZE}*${GRADIENT_ACCUMULATION_STEPS}*${NUM_GPUS}_chunk${CHUNK_SIZE}_tb${NUM_TRANSFORMER_BLOCKS}_mlp${NUM_MLP_LAYERS}_${STEPS}steps_layer${WHICH_LAYER}_$(date +%Y%m%d)_wp${WANDB_PROJECT}_${DATASET_NAME}_USE_SHARED_CACHE_${USE_SHARED_CACHE}_CACHE_VLM_STATES_${CACHE_VLM_STATES}_USE_AMP_${USE_AMP}_webdataset_${USE_WEBDATASET}/logs"

# Wandb 运行名称
WANDB_RUN_NAME="${MODEL_NAME}_layer${WHICH_LAYER}_${GPU_NAME}_bsz${BATCH_SIZE}*${GRADIENT_ACCUMULATION_STEPS}*${NUM_GPUS}_chunk${CHUNK_SIZE}_tb${NUM_TRANSFORMER_BLOCKS}_mlp${NUM_MLP_LAYERS}_${STEPS}steps_paracat_$(date +%Y%m%d)_${DATASET_NAME}_USE_SHARED_CACHE_${USE_SHARED_CACHE}_CACHE_VLM_STATES_${CACHE_VLM_STATES}_USE_AMP_${USE_AMP}_webdataset_${USE_WEBDATASET}"
...
TRAIN_SCRIPT="${PROJECT_ROOT}/Action_Heads/ParaCAT/train_multigpu_only_paracat.py"
...
echo "============================================================================"
echo "🐱 ParaCAT Action Head 训练 (Only ParaCAT)"
echo "============================================================================"
echo ""
echo "📁 路径配置:"
echo "  数据集: ${DATA_PATH}"
...
if [ -n "${PONS_CHECKPOINT}" ]; then
    echo "🔗 Pons 配置:"
    echo "  Checkpoint: ${PONS_CHECKPOINT}"
    echo "  Q Seq Len: ${PONS_Q_SEQ_LEN}"
    echo "  Num Blocks: ${PONS_NUM_BLOCKS}"
    echo "  Freeze Pons: ${FREEZE_PONS}"
    echo ""
else
    echo "🔗 Pons: 不使用 (直接使用 VLM hidden states)"
    echo ""
fi
echo "📊 离散化配置:"
