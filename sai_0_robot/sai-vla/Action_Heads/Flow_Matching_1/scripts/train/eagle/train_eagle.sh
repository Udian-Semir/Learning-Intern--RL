...
# ============================================================================
# Flow Matching 1 Action Head - Eagle 2.5 VL 训练脚本
# ============================================================================
#
# 使用说明:
#   1. 修改下面的配置参数（数据集路径、GPU、训练参数等）
#   2. 运行: bash train_eagle.sh
#
# 支持:
#   - 多GPU分布式训练 (使用 torchrun)
#   - Eagle 2.5 VL (GR00T-N1.5-3B) VLM hidden states
#   - 混合精度训练 (AMP)
#   - 梯度累积
#   - Wandb 日志记录
#
...
# 数据集路径 (必填)
# 需要包含 Eagle VLM hidden states，格式: (batch, num_layers, seq_len, hidden_dim)
DATA_PATH="/data/HuangWenlong/datasets/eagle_extract_-1_layer_hidden_state/libero_github_convert_for_eagle2-5-only-libero_spatial"
...
WHICH_LAYER="last" # 只在文件命名起作用，不影响实际训练
MODEL_NAME="eagle2-5" # 只在文件命名起作用，不影响实际训练 # qwen3-2b, qwen3-4b, qwen3-8b, qwen3-32b, eagle2-5
...
#       --input_path /data/.../libero_github_convert_for_eagle2-5-only-libero_spatial \
...
#   /data/.../libero_github_convert_for_eagle2-5-only-libero_spatial/webdataset_shard5000_ac25/
#   ├── shard-000000.tar
#   ├── shard-000001.tar
#   ├── ...
#   └── meta.json
...
WEBDATASET_SHARD_PATTERN="/data/HuangWenlong/datasets/eagle_extract_-1_layer_hidden_state/libero_github_convert_for_eagle2-5-only-libero_spatial/webdataset_shard5000_ac16/shard-{000000..000010}.tar"
...
WANDB_PROJECT="flow_matching_1_eagle_libero"

# Checkpoint 输出目录
OUT_DIR="/data/HuangWenlong/datasets/eagle_extract_hidden_state/libero_github_convert_for_eagle-only-libero_spatial_weight/Flow_Matching_1/1layer_last/${GPU_NAME}_bsz${BATCH_SIZE}*${GRADIENT_ACCUMULATION_STEPS}*${NUM_GPUS}_${STEPS}steps_${NUM_ACTION_CHUNKS}chunks_layer${WHICH_LAYER}_$(date +%Y%m%d)_wp${WANDB_PROJECT}_${DATASET_NAME}_USE_SHARED_CACHE_${USE_SHARED_CACHE}_CACHE_VLM_STATES_${CACHE_VLM_STATES}_USE_AMP_${USE_AMP}_webdataset_${USE_WEBDATASET}/checkpoints"

# 日志输出目录
LOG_DIR="/data/HuangWenlong/datasets/eagle_extract_hidden_state/libero_github_convert_for_eagle-only-libero_spatial_weight/Flow_Matching_1/1layer_last/${GPU_NAME}_bsz${BATCH_SIZE}*${GRADIENT_ACCUMULATION_STEPS}*${NUM_GPUS}_${STEPS}steps_${NUM_ACTION_CHUNKS}chunks_layer${WHICH_LAYER}_$(date +%Y%m%d)_wp${WANDB_PROJECT}_${DATASET_NAME}_USE_SHARED_CACHE_${USE_SHARED_CACHE}_CACHE_VLM_STATES_${CACHE_VLM_STATES}_USE_AMP_${USE_AMP}_webdataset_${USE_WEBDATASET}/logs"

# Wandb 运行名称 (留空则自动生成)
WANDB_RUN_NAME="${MODEL_NAME}_layer${WHICH_LAYER}_${GPU_NAME}_bsz${BATCH_SIZE}*${GRADIENT_ACCUMULATION_STEPS}*${NUM_GPUS}_${STEPS}steps_fm1_$(date +%Y%m%d)_${DATASET_NAME}_USE_SHARED_CACHE_${USE_SHARED_CACHE}_CACHE_VLM_STATES_${CACHE_VLM_STATES}_USE_AMP_${USE_AMP}_webdataset_${USE_WEBDATASET}"
...
# 训练脚本路径
TRAIN_SCRIPT="${PROJECT_ROOT}/Action_Heads/Flow_Matching_1/train_multigpu.py"
...
echo "============================================================================"
echo "🦅 Flow Matching 1 Action Head - Eagle 2.5 VL 训练"
echo "============================================================================"
echo ""
echo "📁 路径配置:"
echo "  数据集: ${DATA_PATH}"
