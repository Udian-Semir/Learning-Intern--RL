...
class TrainingConfig:
    ...
    """Action backbone 维度 (默认 1536 对应 Qwen 2B)
    Action backbone dimension (default 1536 for Qwen 2B)
    - Qwen 2B: 1536
    - Qwen 4B: 2560 (需要设置 vlm_output_dim=2560)
    """
    
    vlm_output_dim: int = 2048
    """VLM 输出维度 (默认 2048)
    VLM output dimension (default 2048)
    - 必须等于 vl_self_attention_head_dim * vl_self_attention_num_attention_heads
    - Qwen 2B: 2048 = 64 * 32
    - Qwen 4B: 2560 = 64 * 40
    """
    ...
    VL self-attention head dimension (default 64)
    """
    
    vl_self_attention_num_attention_heads: int = 32
    """VL self-attention head 数量 (默认 32)
    VL self-attention head count (default 32)
    - Qwen 2B: 32 (2048 / 64 = 32)
    - Qwen 4B: 40 (2560 / 64 = 40)
    """
...
def lerobot_collate_fn(batch, max_state_dim=64, max_action_dim=32, normalizers=None, 
    ...
    # 处理 VLM hidden states: 将多层在序列维度上合并
    # (batch, num_layers, seq_len, hidden_dim) -> (batch, seq_len * num_layers, hidden_dim)
    if vlm_tensor_raw.dim() == 4:
        batch_size_vlm, num_layers, seq_len_per_layer, hidden_dim = vlm_tensor_raw.shape
        if num_layers > 1:
            # 多层：将每一层提取出来组成 list，然后在序列维度合并
            # 合并后形状: (batch, seq_len * num_layers, hidden_dim)
            layer_list = [vlm_tensor_raw[:, i, :, :] for i in range(num_layers)]
            vlm_tensor = merge_hidden_states_in_seq_dim(layer_list, dim=1)
            '''合并后形状: (batch, seq_len * num_layers, hidden_dim)
            [layer0_token0, layer0_token1, ..., layer0_token511,  ← 第0层
             layer1_token0, layer1_token1, ..., layer1_token511,  ← 第1层  
             layer2_token0, layer2_token1, ..., layer2_token511]  ← 第2层
            '''
        else:
            # 单层：直接取出
...
示例 Examples:
  ...
  python train_with_pretrained.py \\
    ...
    parser.add_argument("--action_backbone_dim", type=int, default=config.action_backbone_dim,
                        help=f"Action backbone 维度 | Action backbone dimension (default: {config.action_backbone_dim})")
    parser.add_argument("--vlm_output_dim", type=int, default=config.vlm_output_dim,
                        help=f"VLM 输出维度 | VLM output dimension (default: {config.vlm_output_dim})")
    parser.add_argument("--vl_self_attention_head_dim", type=int, default=config.vl_self_attention_head_dim,
                        help=f"VL self-attention head 维度 | VL self-attention head dim (default: {config.vl_self_attention_head_dim})")
    parser.add_argument("--vl_self_attention_num_attention_heads", type=int, default=config.vl_self_attention_num_attention_heads,
                        help=f"VL self-attention head 数量 | VL self-attention head count (default: {config.vl_self_attention_num_attention_heads})")
    ...
    vl_self_attention_head_dim = args.vl_self_attention_head_dim
    vl_self_attention_num_attention_heads = args.vl_self_attention_num_attention_heads
    max_action_dim = args.max_action_dim
    action_horizon = args.num_action_chunks
    max_state_dim = args.max_state_dim
    
    # 参数一致性检查
    expected_vlm_output_dim = vl_self_attention_num_attention_heads * vl_self_attention_head_dim
    if vlm_output_dim != expected_vlm_output_dim:
        print_rank0(f"⚠️ 警告: vlm_output_dim ({vlm_output_dim}) 不等于 vl_self_attention_num_attention_heads * vl_self_attention_head_dim ({expected_vlm_output_dim})", rank)
        print_rank0(f"  建议: 设置 --vlm_output_dim {expected_vlm_output_dim} 或调整 attention head 参数", rank)
    else:
        print_rank0(f"✓ 参数一致性检查通过: vlm_output_dim = {vlm_output_dim} = {vl_self_attention_num_attention_heads} × {vl_self_attention_head_dim}", rank)
    ...
    cfg = get_flowmatching_action_head_config(
        action_backbone_dim=action_backbone_dim,
        vlm_output_dim=vlm_output_dim,
        vl_self_attention_head_dim=vl_self_attention_head_dim,
        vl_self_attention_num_attention_heads=vl_self_attention_num_attention_heads,
    ...
    if wandb_initialized and is_main_process(rank):
        try:
            ...
            wandb.config.update({
                ...
                "model/action_backbone_dim": action_backbone_dim,
                "model/vlm_output_dim": vlm_output_dim,
                "model/vl_self_attention_head_dim": vl_self_attention_head_dim,
                "model/vl_self_attention_num_attention_heads": vl_self_attention_num_attention_heads,
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            # 使用自定义 collate function 转换格式 (包含归一化)
            ...
            if (batch_idx + 1) % gradient_accumulation_steps == 0:
                # 先 unscale 梯度，然后计算梯度范数
                scaler.unscale_(optim)
                
                # 计算梯度范数（在 optimizer.step() 之前）
                ...
                scaler.step(optim)
                scaler.update()
                scheduler.step()  # 更新学习率 (cosine decay with warmup)
                optim.zero_grad(set_to_none=True)
