...
def main():
    ...
    parser.add_argument("--num_bf_generations", type=int, default=3, help="生成随机输入时的bf生成次数（仅在未提供npz时生效）")
    ...
    print(f"Config: action_dim={cfg.action_dim}, action_horizon={cfg.action_horizon}")
    print(f"        backbone_embedding_dim={cfg.backbone_embedding_dim}")
    ...
    if args.npz:
        ...
        print(f"Loaded backbone_features shape: {bf.shape}")
        print(f"Loaded state shape: {state.shape}")
    else:
        # 生成随机输入（支持多次生成拼接）
        S = 256
        backbone_dim = cfg.backbone_embedding_dim
        total_seq_len = S * args.num_bf_generations
        print(f"\nGenerating random input with {args.num_bf_generations} generations")
        print(f"backbone_features shape: (1, {total_seq_len}, {backbone_dim})")
        bf = torch.randn(1, total_seq_len, backbone_dim, device=args.device)
        bmask = torch.ones(1, total_seq_len, dtype=torch.long, device=args.device)
...
if __name__ == "__main__":
    ...
    # python infer_once.py --ckpt_dir ./experiments/pickupanapple_v1/checkpoints/best --device cuda:7
    # python infer_once.py --ckpt_dir ./fm1_ckpts_finetuned --npz /path/to/sample.npz
    # python infer_once.py --ckpt_dir ./fm1_ckpts --num_bf_generations 3  # 指定生成次数
    # 
    # 方法 2: 直接设置参数运行（用于测试）
    import sys
    sys.argv = [
        'infer_once.py',
        '--ckpt_dir', '/home/sythoid_01/文档/Huangwenlong/n1.5-split/gr00t_split/Action_Heads/Flow_Matching_1/experiments/pickupanapple_v1_hidden_dim_3_512_2048/checkpoints/epoch_0',
        '--device', 'cuda:7',
        '--num_bf_generations', '3',  # 生成3次backbone_features并拼接
