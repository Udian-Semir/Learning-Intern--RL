...
DATA_PATH="/home/dev/文档/huangwenlong/vla-data-pipeline/alignment_data_post_process2lerobot/datasets/dataset196_rightarm"
...
DATASET_NAME="datasets196_joint_chunk1000_60000_bsz1000"
...
WANDB_PROJECT="datasets196_joint"

# ===== Resume 配置 =====
# 支持填写 step_xxx / epoch_xxx 目录，或其中的 training_state.pt / action_head.pt 文件
RESUME_FROM_CHECKPOINT="/home/dev/文档/huangwenlong/sai_vla_checkpoint/p6000_bsz1000*1*8_tb4_100000steps_layer14_20260403_wpdatasets196_joint_datasets196_joint_chunk1000_60000_bsz1000_USE_SHARED_CACHE_true_CACHE_VLM_STATES_false_USE_AMP_true_webdataset_false/checkpoints/step_100000/"

# true: 恢复时继续写回原来的 checkpoints/logs 目录
# false: 恢复权重和训练状态，但输出到下面新生成的日期目录
RESUME_USE_EXISTING_DIRS="false"

# 新实验根目录（仅在不复用旧目录时使用）
RUN_ROOT_DEFAULT="/home/dev/文档/huangwenlong/sai_vla_checkpoint_resume/${GPU_NAME}_bsz${BATCH_SIZE}*${GRADIENT_ACCUMULATION_STEPS}*${NUM_GPUS}_tb${NUM_TRANSFORMER_BLOCKS}_${STEPS}steps_layer${WHICH_LAYER}_$(date +%Y%m%d)_wp${WANDB_PROJECT}_${DATASET_NAME}_USE_SHARED_CACHE_${USE_SHARED_CACHE}_CACHE_VLM_STATES_${CACHE_VLM_STATES}_USE_AMP_${USE_AMP}_webdataset_${USE_WEBDATASET}"

# 根据是否 resume 决定输出目录
if [ -n "${RESUME_FROM_CHECKPOINT}" ]; then
    if [ -d "${RESUME_FROM_CHECKPOINT}" ]; then
        RESUME_CHECKPOINT_DIR="${RESUME_FROM_CHECKPOINT}"
    else
        RESUME_CHECKPOINT_DIR="$(dirname "${RESUME_FROM_CHECKPOINT}")"
    fi

    if [ ! -d "${RESUME_CHECKPOINT_DIR}" ]; then
        echo "❌ Resume checkpoint 不存在: ${RESUME_CHECKPOINT_DIR}"
        exit 1
    fi

    if [ "${RESUME_USE_EXISTING_DIRS}" = "true" ]; then
        if [ "$(basename "${RESUME_CHECKPOINT_DIR}")" = "checkpoints" ]; then
            OUT_DIR="$(cd "${RESUME_CHECKPOINT_DIR}" && pwd)"
            RUN_ROOT="$(cd "${RESUME_CHECKPOINT_DIR}/.." && pwd)"
        else
            OUT_DIR="$(cd "${RESUME_CHECKPOINT_DIR}/.." && pwd)"
            RUN_ROOT="$(cd "${RESUME_CHECKPOINT_DIR}/../.." && pwd)"
        fi
...
if [ -n "${RESUME_FROM_CHECKPOINT}" ]; then
    ...
    echo "  Reuse Existing Dirs: ${RESUME_USE_EXISTING_DIRS}"
fi
echo ""
echo "💻 系统配置:"
