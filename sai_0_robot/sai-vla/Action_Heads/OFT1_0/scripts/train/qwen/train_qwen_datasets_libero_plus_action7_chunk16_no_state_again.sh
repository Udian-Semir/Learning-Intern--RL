...
# ============================================================================
# OFT1_0 Action Head - Qwen VL 训练脚本
# ============================================================================
#
# 使用说明:
...
#   - Qwen2-VL-7B VLM hidden states
#   - 混合精度训练 (AMP)
#   - 梯度累积
#   - Wandb 日志记录
#
# Qwen VL 参数说明:
#   - VLM 输出维度: 4096 (Qwen2-VL-7B)
#   - 默认提取层: -1 (最后一层) 或中间层
#
# ============================================================================
...
#
# libero_plus_lerobot 信息:
#   - 14347 episodes / 2.24M frames / 15 chunks (chunks_size=1000)
#   - vlm_hidden_states 总量 ~2.6 TiB (fp32)
#   - action 7-dim:  [delta_x, delta_y, delta_z, delta_roll, delta_pitch, delta_yaw, gripper]
#   - state  8-dim:  [eef_pos_xyz, eef_euler_rpy, gripper_qpos_left, gripper_qpos_right]
#
# ⚠️ 必须用 NVMe (/data_disk2/...)！实测：
#     /data_disk1 = HDD（机械盘），顺序读 ~50 MB/s → 单批装载需 4+ 小时
#     /data_disk2 = NVMe SSD，    顺序读 3+ GB/s   → 单批装载约 5 分钟
DATA_PATH="/data_disk2/hwl/datasets/libero_plus_lerobot"

# ============================================================================
# 数据集维度对齐 (覆盖 Action_Heads/OFT1_0/constants.py 的默认值)
# ----------------------------------------------------------------------------
# constants.py 已改造：当前进程读到这些 export 就会用我们的值，否则回退默认 8。
# libero_plus 的 schema (来自 meta/info.json):
#   - action: 7-dim   [delta_x, delta_y, delta_z, delta_roll, delta_pitch, delta_yaw, gripper]
#   - state : 8-dim   [eef_pos_xyz(3), eef_euler_rpy(3), gripper_qpos_left, gripper_qpos_right]
# 注意：PROPRIO_DIM=8 与 constants.py 默认值一致，但仍显式 export，便于 grep/审计。
# ============================================================================
export ACTION_DIM=7
export PROPRIO_DIM=8
#
# ⚠️ NUM_ACTIONS_CHUNK 必须根据数据集的 episode 长度调整！
#   libero_plus 统计 (来自 meta/episodes.jsonl)：
#     fps      = 20
#     mean ep  = 156 帧 / 7.8 秒
#     p99 ep   = 367 帧 / 18.4 秒
#     max ep   = 505 帧 / 25.25 秒
#   常见档位下"GT chunk 越界 → padding"的占比：
#     chunk=8   (0.4s)  → 4.5%  padding (OpenVLA/OFT 默认)
#     chunk=16  (0.8s)  → 9.6%  padding (推荐起步)
#     chunk=50  (2.5s)  → 31%   padding (推荐中程规划)
#     chunk=1000(50.0s) → 91%   padding (此前默认值，几乎全是 padding，浪费!)
#   迁移到短任务数据集时务必下调，否则训练信号被稀释 + 显存白吃。
...
# ============================================================================
# 用 unitree_v2_pretrain_shuffleagain_then_tele 的 step_100000 作为预训练初始化，
# 在 libero_plus 上重新走完整的 warmup + cosine（step / optimizer / scheduler 全部从 0 起）。
RESUME_FROM_CHECKPOINT="/data_disk2/hwl/checkpoints/unitree_v2_pretrain_shuffleagain_then_tele_reset_lr_step100000/action7_chunk16/p6000_bsz500*1*8_tb4_100000steps_layer14_20260519_wpunitree_v2_pretrain_unitree_v2_pretrain_then_tele_CBC_false_dtypefloat32_USE_AMP_true_MD_true/checkpoints/step_100000"
...
WANDB_PROJECT="libero_plus"

# Checkpoint 输出目录
OUT_DIR="/data_disk2/hwl/checkpoints/libero_plus_lerobot/action7_chunk16_pretrain/${GPU_NAME}_bsz${BATCH_SIZE}*${GRADIENT_ACCUMULATION_STEPS}*${NUM_GPUS}_tb${NUM_TRANSFORMER_BLOCKS}_${STEPS}steps_layer${WHICH_LAYER}_${RUN_DATE}_wp${WANDB_PROJECT}_${DATASET_NAME}_CBC_${USE_CHUNK_BATCH_CACHE}_dtype${CHUNK_BATCH_CACHE_DTYPE}_USE_AMP_${USE_AMP}/checkpoints"

# 日志输出目录
LOG_DIR="/data_disk2/hwl/checkpoints/libero_plus_lerobot/action7_chunk16_pretrain/${GPU_NAME}_bsz${BATCH_SIZE}*${GRADIENT_ACCUMULATION_STEPS}*${NUM_GPUS}_tb${NUM_TRANSFORMER_BLOCKS}_${STEPS}steps_layer${WHICH_LAYER}_${RUN_DATE}_wp${WANDB_PROJECT}_${DATASET_NAME}_CBC_${USE_CHUNK_BATCH_CACHE}_dtype${CHUNK_BATCH_CACHE_DTYPE}_USE_AMP_${USE_AMP}/logs"

# Wandb 运行名称 (留空则自动生成)
WANDB_RUN_NAME="${MODEL_NAME}_layer${WHICH_LAYER}_${GPU_NAME}_bsz${BATCH_SIZE}*${GRADIENT_ACCUMULATION_STEPS}*${NUM_GPUS}_tb${NUM_TRANSFORMER_BLOCKS}_${STEPS}steps_oft1_${RUN_DATE}_${DATASET_NAME}_CBC_${USE_CHUNK_BATCH_CACHE}_dtype${CHUNK_BATCH_CACHE_DTYPE}_USE_AMP_${USE_AMP}"
...
# 数据加载模式优先级: ChunkBatchCache > WebDataset 分批缓存 > WebDataset > 共享内存缓存 > mmap
