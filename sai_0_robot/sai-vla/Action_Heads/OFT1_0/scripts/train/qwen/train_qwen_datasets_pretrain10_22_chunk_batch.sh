...
# Checkpoint 输出目录
OUT_DIR="/home/dev/文档/huangwenlong/sai_vla_checkpoint/${GPU_NAME}_bsz${BATCH_SIZE}*${GRADIENT_ACCUMULATION_STEPS}*${NUM_GPUS}_tb${NUM_TRANSFORMER_BLOCKS}_${STEPS}steps_layer${WHICH_LAYER}_$(date +%Y%m%d)_wp${WANDB_PROJECT}_${DATASET_NAME}_CBC_${USE_CHUNK_BATCH_CACHE}_dtype${CHUNK_BATCH_CACHE_DTYPE}_USE_AMP_${USE_AMP}/checkpoints"

# 日志输出目录
LOG_DIR="/home/dev/文档/huangwenlong/sai_vla_checkpoint/${GPU_NAME}_bsz${BATCH_SIZE}*${GRADIENT_ACCUMULATION_STEPS}*${NUM_GPUS}_tb${NUM_TRANSFORMER_BLOCKS}_${STEPS}steps_layer${WHICH_LAYER}_$(date +%Y%m%d)_wp${WANDB_PROJECT}_${DATASET_NAME}_CBC_${USE_CHUNK_BATCH_CACHE}_dtype${CHUNK_BATCH_CACHE_DTYPE}_USE_AMP_${USE_AMP}/logs"

# Wandb 运行名称 (留空则自动生成)
WANDB_RUN_NAME="${MODEL_NAME}_layer${WHICH_LAYER}_${GPU_NAME}_bsz${BATCH_SIZE}*${GRADIENT_ACCUMULATION_STEPS}*${NUM_GPUS}_tb${NUM_TRANSFORMER_BLOCKS}_${STEPS}steps_oft1_$(date +%Y%m%d)_${DATASET_NAME}_CBC_${USE_CHUNK_BATCH_CACHE}_dtype${CHUNK_BATCH_CACHE_DTYPE}_USE_AMP_${USE_AMP}"
