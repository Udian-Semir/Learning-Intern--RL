"""
Eagle 2.5 VL Backbone Module

提供 Eagle 2.5 VL 模型的 hidden states 提取功能。
可独立使用，也可与 Action Head 组合形成完整的 VLA 系统。

核心组件:
- Eagle25VLBackbone: VLM backbone 主类
- Eagle25VLConfig: 配置类
- HiddenStateOutput: hidden states 输出封装

使用示例:
    # 1. 基本使用
    from VLMs.S0_1.backbone.eagle2_5_vl import Eagle25VLBackbone
    ...
    # 4. 自定义配置
    from VLMs.S0_1.backbone.eagle2_5_vl import Eagle25VLConfig
    ...
    backbone = Eagle25VLBackbone(config=config)
"""
...
def create_backbone(
    ...
    **kwargs
    ...
    """
    创建 Eagle 2.5 VL Backbone 实例
    
    这是最简单的创建方式，适合快速开始。
    
    Args:
        model_path: 模型路径 (本地路径或 HuggingFace ID)
        layers: 要提取的层号，默认 [-4] (倒数第4层)
        device: 设备
        **kwargs: 其他配置参数
        
    Returns:
        Eagle25VLBackbone 实例
...
def create_backbone_for_model(
    ...
    **kwargs
    ...
    Example:
        ...
        >>> backbone = create_backbone_for_model("/path/to/model", layers=[-4, -3])
    """
...
def connect_vlm_to_action_head(
    vlm_backbone: Eagle25VLBackbone,
    action_head,
...
def is_eagle25vl_backbone(obj) -> bool:
    """检查对象是否是 Eagle25VL Backbone"""
    return isinstance(obj, Eagle25VLBackbone)


def get_backbone_info(backbone: Eagle25VLBackbone) -> dict:
    """获取 backbone 信息"""
