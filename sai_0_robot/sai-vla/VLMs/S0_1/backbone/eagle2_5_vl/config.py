...
使用示例:
    ...
    config = Eagle25VLConfig(
        model_path="/path/to/eagle2_5_vl_model",
        layers=[-4],
...
# 默认 Eagle 模型路径
DEFAULT_EAGLE_MODEL_PATH = Path(__file__).parent / "ori" / "backbone" / "eagle2_hg_model"
...
class ModelConfig:
    ...
    default_layers: List[int] = field(default_factory=lambda: [-4])
...
class Eagle25VLConfig:
    ...
    # 本地 Eagle 模型配置路径 (eagle2_hg_model 目录)
    eagle_config_path: Optional[str] = None
    ...
    def __post_init__(self):
        ...
        # 如果 eagle_config_path 未指定，使用默认路径
        if self.eagle_config_path is None:
            self.eagle_config_path = str(DEFAULT_EAGLE_MODEL_PATH)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "eagle_config_path": self.eagle_config_path,
            "device": self.device,
            "dtype": self.dtype,
            "layers": self.layers,
    ...
    @classmethod
    def from_dict(cls, config_dict: Dict[str, Any]) -> "Eagle25VLConfig":
        """从字典创建配置"""
...
def load_config(
    ...
    **overrides
    ...
    # Eagle 特定配置
    if "eagle" in yaml_config:
        eagle_cfg = yaml_config["eagle"]
        config_dict["use_pixel_shuffle"] = eagle_cfg.get("use_pixel_shuffle")
        config_dict["downsample_ratio"] = eagle_cfg.get("downsample_ratio")
        config_dict["mlp_connector_layers"] = eagle_cfg.get("mlp_connector_layers")
        config_dict["min_dynamic_tiles"] = eagle_cfg.get("min_dynamic_tiles")
        config_dict["max_dynamic_tiles"] = eagle_cfg.get("max_dynamic_tiles")
        config_dict["use_thumbnail"] = eagle_cfg.get("use_thumbnail")
    ...
    # 5. 创建配置对象
    return Eagle25VLConfig(**config_dict)
...
def get_default_config() -> Eagle25VLConfig:
    """获取默认配置（不加载 YAML 文件）"""
    return Eagle25VLConfig()


def get_config_for_model(model_path: str, **overrides) -> Eagle25VLConfig:
    """
    根据模型路径获取配置
