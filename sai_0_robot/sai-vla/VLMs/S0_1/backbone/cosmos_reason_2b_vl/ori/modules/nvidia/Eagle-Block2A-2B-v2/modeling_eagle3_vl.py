...
(Siglip2VisionModel,) = _import_local_module("modeling_siglip2", ["Siglip2VisionModel"])
(Eagle3_VLConfig,) = _import_local_module("configuration_eagle3_vl", ["Eagle3_VLConfig"])
    ...
    def __init__(self, config: Eagle3_VLConfig, vision_model=None, language_model=None):
        ...
        if language_model is not None:
            ...
            if config.text_config.architectures[0] == 'LlamaForCausalLM':
                ...
                # MODIFIED: 移除强制 flash_attention_2 要求，改为警告
                # 原始代码:
                ...
                # 新代码:
                if hasattr(config.text_config, '_attn_implementation') and config.text_config._attn_implementation != 'flash_attention_2':
                    import warnings
                    warnings.warn(f"Qwen2 recommends flash_attention_2 but got {config.text_config._attn_implementation}. Performance may be affected.")
                ...
                if hasattr(config.text_config, '_attn_implementation') and config.text_config._attn_implementation != 'flash_attention_2':
                    import warnings
                    warnings.warn(f"Qwen3 recommends flash_attention_2 but got {config.text_config._attn_implementation}. Performance may be affected.")
