...
try:
    ...
    # 当作为独立模块加载时，从 sys.modules 获取
    if 'modeling_siglip2' in sys.modules:
        Siglip2VisionConfig = sys.modules['modeling_siglip2'].Siglip2VisionConfig
    else:
        # 尝试从同目录导入
        from pathlib import Path
        import importlib.util
        current_dir = Path(__file__).parent
        spec = importlib.util.spec_from_file_location("modeling_siglip2", current_dir / "modeling_siglip2.py")
        siglip2_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(siglip2_module)
        Siglip2VisionConfig = siglip2_module.Siglip2VisionConfig
