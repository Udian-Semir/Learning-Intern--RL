...
try:
    ...
    # VideoInput 在旧版本 transformers 中不存在，定义为 Any
        ...
        def replace_in_text(text):
            ...
            def repl(match):
                ...
                if media_type == 'image':
                    ...
                    # MODIFIED: 兼容不同的 image_processor 返回格式
                    # 原始代码期望 image_inputs['image_sizes'] 存在
                    # 新代码: 如果不存在，从 pixel_values 推断
                    if 'image_sizes' in image_inputs:
                        image_height, image_width = image_inputs['image_sizes'][0]
                    else:
                        # 从 pixel_values 推断尺寸 (格式: [batch, channels, height, width])
                        pv = image_inputs['pixel_values']
                        if hasattr(pv, 'shape'):
                            if len(pv.shape) == 4:
                                image_height, image_width = pv.shape[2], pv.shape[3]
                            elif len(pv.shape) == 3:
                                image_height, image_width = pv.shape[1], pv.shape[2]
                            else:
                                image_height, image_width = 384, 384  # 默认值
                        else:
                            image_height, image_width = 384, 384
                        # 添加 image_sizes 以便后续使用
                        image_inputs['image_sizes'] = torch.tensor([[image_height, image_width]])
