...
class Siglip2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        #     )
        # 新代码: 使用 hasattr 检查属性存在性
        elif hasattr(module, 'logit_scale') and hasattr(module, 'logit_bias'):
            # Siglip2Model-like initialization
            logit_scale_init = torch.log(torch.tensor(1.0))
            module.logit_scale.data.fill_(logit_scale_init)
            module.logit_bias.data.zero_()
        elif hasattr(module, 'classifier') and hasattr(self.config, 'vision_config'):
            # Siglip2ForImageClassification-like initialization
            nn.init.normal_(
                module.classifier.weight,
                std=self.config.vision_config.hidden_size**-0.5 * self.config.initializer_factor,
            )
