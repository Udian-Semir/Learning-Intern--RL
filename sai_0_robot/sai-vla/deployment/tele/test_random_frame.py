...
def main():
    parser = argparse.ArgumentParser(description='从视频随机挑选一帧发送给 server')
    parser.add_argument(
        '--video', 
        type=str, 
    ...
    try:
        ...
        for i in range(min(5, len(actions))):
            ...
            discrete_str = ', '.join([str(v) for v in discrete_actions[i][:7]])
            print(f"  Step {i}: [{action_str}...] (discrete: [{discrete_str}...])")
