...
def main():
    ...
    parser = argparse.ArgumentParser(description='ParaCAT 客户端测试')
    ...
    # 创建客户端
    client = ParaCATClient(args.server)
    ...
    try:
        ...
        print(f"  Use Pons: {info['use_pons']}")
        print(f"  Use State Mapper: {info['use_state_mapper']}")
    except Exception as e:
        print(f"获取模型信息失败: {e}")
    ...
    try:
        ...
        print(f"    Pons: {result['timing']['pons_time']:.4f}s")
        print(f"    ParaCAT: {result['timing']['paracat_time']:.4f}s")
