...
PROJECT_ROOT = SCRIPT_DIR.parents[3]  # utils/extract_vlm_hidden_state/S0_1/cosmos -> project root
...
# Cosmos Reason 2B VL (Eagle-Block2A-2B-v2) 默认模型路径
# 使用相对于项目根目录的路径
DEFAULT_COSMOS_MODEL_PATH = str(
    PROJECT_ROOT / "VLMs" / "S0_1" / "backbone" / "cosmos_reason_2b_vl" / 
    "ori" / "modules" / "nvidia" / "Eagle-Block2A-2B-v2"
)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Cosmos Reason 2B VL Hidden States 提取工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
    # 基本用法
    python cosmos_extract_vlm_hidden_states.py --dataset_path /path/to/dataset
    ...
    # 模型配置
    parser.add_argument(
        "--model_path", type=str, default=DEFAULT_COSMOS_MODEL_PATH,
        help=f"Cosmos 模型路径 (默认: Eagle-Block2A-2B-v2)"
    )
    parser.add_argument(
...
def main():
    ...
    # 打印 Banner
    print("\n" + "=" * 70)
    print("🌌 Cosmos Reason 2B VL Hidden States 提取工具")
    print("=" * 70)
    ...
    # 打印配置信息
    print(f"\n📋 配置信息:")
    print(f"  模型类型: Cosmos Reason 2B VL (Eagle-Block2A-2B-v2)")
    ...
    # 创建 Cosmos Reason 2B VL backbone
    print("\n🚀 创建 Cosmos Reason 2B VL Backbone...")
    ...
    try:
        backbone = create_vlm_backbone(
            model_type="cosmos_reason_2b_vl",
    ...
    extract_args.model_type = "cosmos_reason_2b_vl"
...
def extract_cosmos_hidden_states(
    dataset_path: str,
    model_path: str = None,
    output_dir: str = None,
    ...
    """
    Python API: 提取 Cosmos Reason 2B VL hidden states
    ...
    if model_path is None:
        model_path = DEFAULT_COSMOS_MODEL_PATH
    if layers is None:
