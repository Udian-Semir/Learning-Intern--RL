#!/bin/bash
# ============================================================================
# Flow Matching Action Head (Flow_Matching_0) LIBERO 评估脚本
# ============================================================================
#
# 使用说明:
#   1. 修改下面的配置参数
...
CHECKPOINT_PATH="/home/dev/文档/huangwenlong/dataset/libero_github_convert_for_Sai0-VLA_libero_spatial/libero_lerobot_spatial_sys0_eagle_-1_weight/FM0/1layer_-1/p6000_bsz80*1*8_20000steps_16chunks_NO_PRETRAIN-true_layer-1_20260122_wpflow_matching_0_eagle_libero_test_newest_libero_spatial_USE_SHARED_CACHE_true_CACHE_VLM_STATES_false_USE_AMP_true_webdataset_false/checkpoints/step_20000/action_head.pt"
...
# VLM 提取层 (单层)
# Qwen2B: "14", Qwen4B: "16", Eagle: "-1", Cosmos: "-1" 或 "-5" (使用负数索引)
VLM_LAYERS="-1"
...
# Action backbone 维度
# Qwen2B: 1536, Qwen4B: 2560, Eagle: 1536, Cosmos: 1536
...
# ===== 加速模型加载 =====
# 注意: 不要设置 HF_HUB_OFFLINE=1 和 TRANSFORMERS_OFFLINE=1 # 如果使用qwen则可以使用，eagle不能使用
# 这些会导致模型加载变慢（缓存验证问题）
...
# 打印配置
echo "============================================================================"
echo "🚀 Flow Matching Action Head (Flow_Matching_0) LIBERO 评估"
echo "============================================================================"
echo ""
echo "📁 路径配置:"
...
for ((RUN_IDX=1; RUN_IDX<=NUM_EVAL_RUNS; RUN_IDX++)); do
    ...
    CMD="CUDA_VISIBLE_DEVICES=${GPU_ID} python -m eval.Sai0_1.libero.Flow_Matching_0.eval_Sai0_1"
