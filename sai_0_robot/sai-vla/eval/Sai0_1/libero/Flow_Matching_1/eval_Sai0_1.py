...
# ============================================================================
# Flow Matching Policy 封装 (多层 VLM hidden states)
# ============================================================================

class FlowMatchingPolicy:
    """
    Flow Matching Policy 封装 (支持多层 VLM hidden states)
    ...
    def __init__(
        ...
        """
        初始化 Flow Matching Policy (多层版本)
        """
        self.device = device
        self.vlm_layers = vlm_layers or [1, 14, 28]  # 默认 3 层
        ...
        print(f"  - 类型: {vlm_type}")
        print(f"  - 提取层: {self.vlm_layers} (共 {len(self.vlm_layers)} 层)")
    ...
    def get_action(self, observation_dict: dict, lang: str) -> np.ndarray:
        ...
        vlm_hidden_states = vlm_output.hidden_states  # List[Tensor], 每层一个
        
        # 拼接多层 hidden states: List[(1, seq_len, hidden_dim)] -> (1, num_layers * seq_len, hidden_dim)
        # Flow Matching 模型期望的格式是把多层拼接在 sequence 维度
...
class EvalConfig:
    ...
    # VLM 相关
    vlm_layers: List[int] = field(default_factory=lambda: [1, 14, 28])
    vlm_output_dim: int = 2048
    action_backbone_dim: int = 1536
    vl_self_attention_head_dim: int = 64
    vl_self_attention_num_attention_heads: int = 32
...
def eval_with_training_data(cfg: EvalConfig):
    ...
    if config_path.exists():
        ...
        model_cfg = get_flowmatching_action_head_config(
            action_backbone_dim=cfg.action_backbone_dim,
            vlm_output_dim=cfg.vlm_output_dim,
            vl_self_attention_head_dim=cfg.vl_self_attention_head_dim,
            vl_self_attention_num_attention_heads=cfg.vl_self_attention_num_attention_heads,
...
def eval_libero(cfg: EvalConfig):
    ...
    print(f"\n{'='*60}")
    print(f"LIBERO 评估 (Flow Matching - 多层 VLM)")
    print(f"{'='*60}")
    print(f"Task Suite: {cfg.task_suite_name}")
    print(f"Total tasks: {num_tasks_in_suite}")
    print(f"VLM Layers: {cfg.vlm_layers} (共 {len(cfg.vlm_layers)} 层)")
    ...
    log_file.write(f"VLM Layers: {cfg.vlm_layers}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='LIBERO Evaluation with Flow Matching Action Head (Flow_Matching_1 - Multi-layer)',
    ...
    # VLM 相关参数
    parser.add_argument("--vlm_layers", type=str, default="1,14,28",
                        help="提取的 VLM 隐藏层 (如 '1,14,28' 或 '16,17,18')")
    ...
    parser.add_argument("--vl_self_attention_head_dim", type=int, default=64,
                        help="VL self-attention head 维度")
    parser.add_argument("--vl_self_attention_num_attention_heads", type=int, default=32,
                        help="VL self-attention head 数量")
