...
# Eagle (GR00T-N1.5-3B): "/home/dev/.cache/huggingface/hub/models--nvidia--GR00T-N1.5-3B/snapshots/869830fc749c35f34771aa5209f923ac57e4564e"
...
# VLM 提取层
# Qwen2B: "14", Qwen4B: "16,17,18", Eagle: "-1", Cosmos: "-1" 或 "-5" (使用负数索引)
# 多层用逗号分隔，如 "14,15,16"
VLM_LAYERS="-1"
...
# 仅将任务描述（{instruction}）转为小写，模板部分保持不变
# 示例: "What action should the robot take to pick up the apple?"
#       ↑ "What" 保持大写，"pick up the apple" 变小写
...
# 添加 "What action should the robot take to {instruction}?" 前缀
...
# 每次执行的动作数
# 设置为 1 表示每步重新预测
# 设置为 NUM_ACTION_CHUNKS 表示执行所有预测动作，否则执行NUM_ACTION_CHUNKS中的ACTION_CHUNK_SIZE步预测
ACTION_CHUNK_SIZE=16

# 是否执行所有预测的动作 (true/false)
# 如果为 true，将执行所有 NUM_ACTION_CHUNKS 步预测动作，如果为 false，将执行ACTION_CHUNK_SIZE步预测
...
# 预测的动作块数量 (action chunking) # 与ACTION_DIM共同决定OFT1_0的reshape方式
...
# GPU 设备 ID
# 使用 CUDA_VISIBLE_DEVICES 选择物理 GPU
GPU_ID=0

# PyTorch 设备
# 由于使用 CUDA_VISIBLE_DEVICES，这里通常设为 "cuda:0"
DEVICE="cuda:0"
...
# 如果为 true，将使用训练数据集进行测试，而非 LIBERO 仿真环境
# 用于验证模型是否正确加载、数据处理流程是否正确
USE_TRAINING_DATA="false"

# 测试样本数量
# 仅在 USE_TRAINING_DATA="true" 时有效
...
# 构建实验目录名
EXPERIMENT_NAME="${VLM_TYPE}_${MODEL_SIZE}_${VLM_LAYERS}_${VLM_OUTPUT_DIM}_${ACTION_PROMPT_TAG}_${NUM_TRIALS_PER_TASK}trials_${TASK_SUITE_NAME}_chunk${ACTION_CHUNK_SIZE}_${STEP_NUM}_${GPU_NAME}_$(date +%Y%m%d)"

# 切换到项目根目录
cd "${PROJECT_ROOT}"

# 设置 robosuite 日志目录（避免 /tmp/robosuite.log 权限问题）
export ROBOSUITE_LOG="${HOME}/.robosuite/robosuite.log"
mkdir -p "${HOME}/.robosuite"

# HuggingFace 镜像加速（国内网络可取消注释）
...
# ===== 加速模型加载 =====
# 设置 CPU 线程数（根据你的 CPU 核心数调整，建议设为核心数）
