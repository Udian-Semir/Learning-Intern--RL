#!/usr/bin/env python3
"""Evaluate a Flow Matching 0 action head inside the LIBERO simulator using training set initial states."""
...
def load_episode_init_state(dataset_path: str, episode_id: int) -> Optional[np.ndarray]:
    """Load the first frame state vector from a specific dataset episode."""
    try:
        dataset = load_from_disk(dataset_path)
    except Exception as exc:  # pragma: no cover - diagnostic path
        print(f"[ERROR] Failed to load dataset at {dataset_path}: {exc}")
        return None

    # Hugging Face datasets can be DatasetDict or Dataset; always use the first split.
    ...
    try:
        episode_data = dataset.filter(lambda x: x.get("episode_index", -1) == episode_id)
    except Exception as exc:  # pragma: no cover - diagnostic path
        print(f"[ERROR] Unable to filter dataset for episode {episode_id}: {exc}")
        return None

    if len(episode_data) == 0:
        print(f"[ERROR] Episode {episode_id} not found in dataset {dataset_path}")
        return None

    first_frame = episode_data[0]
    state_key = "observation.state"
    if state_key not in first_frame:
        print(f"[ERROR] Key '{state_key}' missing in episode {episode_id}")
        return None

    init_state = np.asarray(first_frame[state_key], dtype=np.float32)
    print(
        f"[INFO] Loaded initial state from dataset episode {episode_id}, "
        f"state shape: {init_state.shape}"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="LIBERO evaluation with Sai0 Flow Matching 0 action head (training set)")
    ...
    parser.add_argument("--dataset-path", type=str, default=None, help="Path to training dataset for loading init states")
    parser.add_argument("--episode-id", type=int, default=None, help="Dataset episode id to clone environment state from")
    ...
    parser.add_argument("--num-rollouts", type=int, default=20, help="Number of sequential rollouts to perform")
...
def main() -> None:
    ...
    print("\n" + "="*60)
    print("LIBERO Evaluation Configuration (Flow Matching 0 - Training Set)")
    print("="*60)
    ...
    if args.dataset_path and args.episode_id is not None:
        print(f"\n[INFO] Loading initial state from dataset episode {args.episode_id}")
        dataset_state = load_episode_init_state(args.dataset_path, args.episode_id)
        if dataset_state is None:
            print("[WARN] Falling back to benchmark-provided init states")
            init_states = benchmark.get_task_init_states(args.task_id)
            use_episode_state = False
        else:
            init_states = dataset_state
            use_episode_state = True
    else:
        init_states = benchmark.get_task_init_states(args.task_id)
        use_episode_state = False
...
'''
Example CLI for training set evaluation:

python eval_trainingset.py \
  --action-head-ckpt /path/to/Flow_Matching_0/experiments/.../checkpoints/best \
  --action-head-version flow_matching_0 \
  --dataset-path /path/to/libero_lerobot_object \
